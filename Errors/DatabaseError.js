...
export default class DatabaseError extends Error {
    constructor(message) {
        ...
        if (typeof message === 'object' && message.code) {
            switch (message.code) {
                case "P1000":
                    errorMessage = "Database authentication failed. Please ensure database credentials are valid.";
                    statusCode = 500;
                    break;
                case "P1001":
                    errorMessage = "Unable to connect to database. Please ensure database server is running.";
                    statusCode = 500;
                    break;
                case "P1002":
                    ...
                    break;
                case "P2000":
                    errorMessage = `Data too long for column: ${message.meta?.target || 'unknown'}`;
                    statusCode = 400;
                    break;
                case "P2002":
                    errorMessage = `Duplicate entry found for: ${message.meta?.target?.join(', ') || 'unknown'}`;
                    statusCode = 409;
                    break;
                case "P2003":
                    errorMessage = `Foreign key validation failed on: ${message.meta?.field_name || 'unknown'}`;
                    statusCode = 400;
                    break;
                case "P2005":
